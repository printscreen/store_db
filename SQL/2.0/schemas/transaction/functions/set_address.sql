...
DROP FUNCTION IF EXISTS set_address(
    ...
    , i_postal_code     CHARACTER VARYING
    , i_country_id      BIGINT
    , i_phone_number    CHARACTER VARYING
    , i_user_id         BIGINT
    , i_active          BOOLEAN
);
...
$_$
    ...
    BEGIN
        v_hash := md5(lower(
                regexp_replace(COALESCE(i_first_name, '') || COALESCE(i_last_name, '') ||
                    COALESCE(i_street, '') || COALESCE(i_unit_number, '') || COALESCE(i_city, '') || 
                    COALESCE(i_state, '') || COALESCE(i_postal_code, '') || COALESCE(i_country_id, 0) || COALESCE(i_phone_number, ''),
                    E'[ \t\n\r]*'::TEXT,
                    ''::TEXT,
                    'g'::TEXT
                ))
        ...
        IF v_old.address_id IS NULL THEN
            SELECT * INTO v_old FROM transaction.address WHERE hash = v_hash AND user_id = i_user_id;
        END IF;
