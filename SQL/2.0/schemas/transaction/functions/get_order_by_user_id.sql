DROP FUNCTION IF EXISTS get_order_by_user_id (
    i_user_id           BIGINT
  , i_sort_field        BIGINT
  , i_offset            BIGINT
  , i_limit             BIGINT
...
CREATE OR REPLACE FUNCTION get_order_by_user_id (
    i_user_id           BIGINT
  , i_sort_field        BIGINT
  , i_offset            BIGINT
  , i_limit             BIGINT
    ...
    BEGIN
        ...
        v_sql := '
            SELECT 
                ...
                ,( SELECT COUNT(*)
                         FROM transaction.v_order o WHERE true '||COALESCE(' AND o.user_id='||i_user_id, '')||'
                       )::BIGINT       AS total
            FROM transaction.v_order o
            WHERE TRUE'
            || COALESCE(' AND o.user_id='||i_user_id, '') || 
