DROP FUNCTION IF EXISTS delete_user_type_resource (
    i_user_type_id      BIGINT
  , i_resource_group_id BIGINT
);
CREATE OR REPLACE FUNCTION delete_user_type_resource (
    i_user_type_id      BIGINT
  , i_resource_group_id BIGINT
)
RETURNS BOOLEAN
...
$_$
    ...
    BEGIN
        ...
        LOOP
            ...
            IF v_find.user_type_resource_id IS NOT NULL THEN
                DELETE FROM users.user_type_resource WHERE user_type_id = i_user_type_id AND resource_id = REC.resource_id;
            END IF;
        END LOOP;
       RETURN TRUE;
    END;
